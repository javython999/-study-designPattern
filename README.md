...
* 인터프리터 패턴의 장/단점
  * 장점
    ...
    * 기존 코드를 변경하지 않고 새로운 Expression을 추가할 수있다.
  * 단점
...
* 이터레이터 패턴의 장/단점
  * 장점
    * 집합 객체가 가지고 있는 객체들에 손쉽게 접근이 가능할 수 있다.
    * 일관된 인터페이스를 사용해 여러 형태의 집합 구조를 순회할 수 있다.
  * 단점
    * 클래스가 늘어나고 복잡도가 증가한다.
***
## 중재자(Mediator)
여러 객채들이 소통하는 방법을 캡슐화 하는 패턴.
...
* 메멘토 패턴의 장/단점
  * 장점
    * 캡슐화를 지키면서 상태 스냅샷을 만들 수 있다.
    * 객체 상태를 저장하고 복원하는 역할을 CareTaker에게 위임할 수 있다.
    * 객체 상태가 바뀌어도 클라이언트 코드는 변경되지 않는다.
  * 단점
    * 많은 정보를 저장하는 Mementor를 자주 생성하는 경우 메모리 사용량에 영향을 줄 수 있다.
***
## 옵저버(Observer)
...
* 옵저버 패턴의 장/단점
  ...
  * 단점
    * 복잡도가 증가한다.
    * 다수의 Observer 객체를 등록 이후 해지하지 않는다면 memory leak이 발생할 수 도 있다.
***
## 상태(State)
